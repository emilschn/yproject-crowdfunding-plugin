	...
	public function get_lemonway_iban() {
		...
		if ( isset( $wallet_details->IBANS->IBAN ) ) {
			if ( is_array( $wallet_details->IBANS->IBAN ) ) {
				...
				if ( count( $wallet_details->IBANS->IBAN ) > 1 && ( $buffer->S == self::$iban_status_disabled || $buffer->S == self::$iban_status_rejected || strtoupper ( str_replace(' ', '', $buffer->HOLDER) ) == strtoupper ( str_replace(' ', '', self::$iban_holder_lw) ) ) ) {
