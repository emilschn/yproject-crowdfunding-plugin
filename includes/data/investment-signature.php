...
class WDGInvestmentSignature {
	...
	private function init_status() {
		if ( $this->investment->get_saved_amount() < self::$investment_amount_signature_needed_minimum ) {
			...
			if ( $this->external_provider == self::$external_provider_signsquid ) {
				$this->status = self::$status_signed;
			} else {
				if ( $this->external_provider == self::$external_provider_eversign ) {
		...
		if ( !empty( $this->external_id ) ) {
			...
			if ( !empty( $document_info ) ) {
				...
				if ( $document_info->is_completed ) {
					...
					if ( $document_info->is_cancelled || $document_info->is_deleted || $document_info->is_expired ) {
