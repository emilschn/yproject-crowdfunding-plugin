	...
	public function get_maximum_investable_amount_without_alert_reason_str() {
		...
		if ( !$this->is_lemonway_registered() ) {
			if ( LemonwayLib::$limit_kyc1_moneyin_operation_amount <= LemonwayLib::$limit_kyc1_moneyin_year_amount - $this->get_count_invested_during_interval( '365 days' ) ) {
				$buffer = sprintf( __( "Vous ne pouvez pas investir plus de %1$s &euro; tant que votre compte n'est pas authentifi&eacute;. Cependant, nous vous proposons de poursuivre votre investissement sans en modifier le montant : vous pourrez alors proc&eacute;der &agrave; un paiement imm&eacute;diat de %1$s &euro;, puis renseigner les documents permettant votre authentification (pi&egrave;ce d'identit&eacute; et justificatif de domicile). Le reste de l'investissement se fera automatiquement de manière s&eacute;curis&eacute;e lors de la validation de vos documents par notre prestataire de paiement Lemon Way.", 'yproject' ), LemonwayLib::$limit_kyc1_moneyin_operation_amount );
