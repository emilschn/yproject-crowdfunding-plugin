...
class WDGUserInvestments
{
	 ...
	 *******************************************************************************/
	public function get_posts_investments($payment_status, $trimestre = false)
	{
		...
		if (!isset(self::$posts_investments[$this->wp_ref][$payment_status_key])) {
			...
			if ($trimestre) {
				...
				self::$posts_investments[$this->wp_ref][$payment_status_key] = get_posts(
					array(
						'numberposts' => -1,
	 ...
	 */
	public function get_investments($payment_status, $trimestre = false)
	{
		$buffer = array();
		$purchases = $this->get_posts_investments($payment_status, $trimestre);
	...
	public function get_validated_investments($trimestre = false)
	{
		...
		return $this->get_investments($payment_status, $trimestre);
	}
		...
		if (!isset($this->pending_preinvestments)) {
			...
			if (!$preinv_cache) {
				...
				foreach ($preinvestment_array as $investment_id) {
					...
					$contract_has_been_modified = ($investment_campaign->contract_modifications() != '');
					if (
						$investment_campaign->campaign_status() == ATCF_Campaign::$campaign_status_collecte
