...
/******************************************************************************/
	...
	public static function shortcode_contract_investor_info( $atts, $content = '' ) {
		...
		$buffer .= 'demeurant ' .$shortcode_investor_user_obj->get_address_number(). ' ' .$shortcode_investor_user_obj->get_address_number_complement(). ' ' .$shortcode_investor_user_obj->get_address(). ' ' .$shortcode_investor_user_obj->get_postal_code( true ). ' ' .$shortcode_investor_user_obj->get_city(). '<br>';
...
function doFillPDFHTMLDefaultContentByLang( $user_obj, $campaign_obj, $payment_data, $organization, $preview, $lang = '', $with_agreement = false ) {
	...
	if ( $user_obj == 'user' ) {
		...
		if ($lang == 'en_US') {
			...
			$buffer .= 'living in '.$WDGUser->get_city().' ('.$WDGUser->get_postal_code( true ).') - ' . $WDGUser->get_address().' ' . $WDGUser->get_address_number(). ' ' .$WDGUser->get_address_number_complement(). '<br />';;
			...
			$buffer .= 'de nationalité '.$nationality.'<br />';
			$buffer .= 'demeurant ' . $WDGUser->get_address_number(). ' ' .$WDGUser->get_address_number_complement(). ' ' . $WDGUser->get_address().' '.$WDGUser->get_postal_code( true ).' '.$WDGUser->get_city().'<br />';;
