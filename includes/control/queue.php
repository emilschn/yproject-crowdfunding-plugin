	...
	public static function execute_vote_authentication_needed_reminder( $user_id, $queued_action_params, $queued_action_id ) {
		...
		if ( !empty( $user_id ) ) {
			...
			if ( !$WDGEntity->has_sent_all_documents() ) {
				NotificationsAPI::vote_authentication_needed_reminder( $user_email, $user_name, $queued_action_params[ 'campaign_name' ], $queued_action_params[ 'campaign_api_id' ] );
			}
...
/******************************************************************************/
	public static function add_vote_authenticated_reminder( $user_id, $user_email, $campaign_name, $campaign_id, $campaign_api_id ) {
		...
		if ( !empty( $user_id ) ) {
			...
			if ( !WDGOrganization::is_user_organization( $user_id ) ) {
				...
				if ( !$WDGEntity->has_invested_on_campaign( $queued_action_params[ 'campaign_id' ] ) ) {
					NotificationsAPI::vote_authenticated_reminder( $user_email, $user_name, $queued_action_params[ 'campaign_name' ], $queued_action_params[ 'campaign_api_id' ] );
				}
			}
