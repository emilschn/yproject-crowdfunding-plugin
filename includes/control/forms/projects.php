		...
		if ( !empty( $approve_payment_id ) && !empty( $campaign_id ) && $current_wdg_user->is_admin() ) {
			...
			if ( $amount > 1500 ) {
				...
				if ($contract_id != '') {
					...
					NotificationsEmails::new_purchase_user_success( $approve_payment_id, $contract_infos->{'signatories'}[0]->{'code'}, FALSE );
					NotificationsEmails::new_purchase_admin_success( $approve_payment_id );
				} else {
					global $contract_errors;
					$contract_errors = 'contract_failed';
					NotificationsEmails::new_purchase_user_error_contract( $approve_payment_id );
				...
				NotificationsEmails::new_purchase_user_success_nocontract( $approve_payment_id, $new_contract_pdf_file, FALSE );
