...
class WDGFormProjects {
	...
	public static function form_validate_news_add($campaign_id) {
		...
		if ( $send_mail == 'on' ) {
			...
			NotificationsAPI::new_project_news( $recipients_string, $replyto_mail, $campaign, $post_campaign->post_title, get_permalink( $campaign_id ), $campaign->get_api_id(), $_POST[ 'posttitle' ], $content_without_links );
		}
	}
