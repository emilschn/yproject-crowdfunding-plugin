	...
	public function payment_return( $mean_of_payment ) {
		...
		if ( $mean_of_payment == WDGInvestment::$meanofpayment_card || $mean_of_payment == WDGInvestment::$meanofpayment_cardwallet ) {
			...
			if ( $buffer == 'failed' ) {
				...
				NotificationsAPI::investment_error( $WDGUser_current->wp_user->user_email, $WDGUser_current->wp_user->user_firstname, $this->get_session_amount(), $this->campaign->data->post_title, $this->error_item->get_error_message( FALSE ) .'<br>'. $lw_transaction_result->INT_MSG, $investment_link );
			}
