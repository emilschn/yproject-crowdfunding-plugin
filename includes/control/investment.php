		...
		if ( $mean_of_payment == WDGInvestment::$meanofpayment_card || $mean_of_payment == WDGInvestment::$meanofpayment_cardwallet ) {
			...
			if ( !$this->exists_payment( $payment_key ) ) {
				...
				if ( $buffer == 'failed' ) {
					...
					NotificationsAPI::investment_error( $WDGUser_current->wp_user->user_email, $WDGUser_current->wp_user->user_firstname, $this->get_session_amount(), $this->campaign->data->post_title, $this->campaign->ID, $this->error_item->get_error_message( FALSE, FALSE ), $investment_link );
