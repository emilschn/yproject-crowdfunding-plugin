...
class WDGEversign {
	...
	private static function call_get($route, $parameters = FALSE) {
		...
		if ( isset( self::$cache_by_route[ $route ] ) ) {
			...
			$result = wp_remote_get($route_to_get, array(
	...
	private static function call_post($route, $parameters, $parameters_get = FALSE) {
		...
		$result = wp_remote_post($route_to_post, array(
				'timeout'	=> self::$http_request_timeout,
				'body'		=> $body
			));
	...
	public static function create_document($title, $message, $wdg_signature_id, $file_name, $file_url, $signer_id, $signer_name, $signer_email, $campaign_id) {
		...
		$redirect_url_accept = WDG_Redirect_Engine::override_get_page_url( 'paiement-partager' ) . '?campaign_id=' .$campaign_id. '&return_eversign=1';
		$redirect_url_decline = WDG_Redirect_Engine::override_get_page_url( 'paiement-partager' ) . '?campaign_id=' .$campaign_id. '&return_eversign=2';
