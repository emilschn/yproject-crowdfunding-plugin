...
class WDGPostActions
{
	...
	public static function create_project_form()
	{
		...
		if (
			!empty($new_firstname) && !empty($new_lastname) && !empty($new_phone)
	...
	public static function generate_campaign_bill()
	{
		...
		if ($WDGUser_current != FALSE && $WDGUser_current->is_admin() && !empty($campaign_id)) {
			$campaign = new ATCF_Campaign($campaign_id);
			$campaign_bill = new WDGCampaignBill($campaign, WDGCampaignBill::$tool_name_pennylane, WDGCampaignBill::$bill_type_crowdfunding_commission);
