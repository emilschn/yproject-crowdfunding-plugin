		...
		if ( $WDGUser_wallet !== FALSE ) {
			...
			if ( $lemonway_posted_wallet_status == 6 ) {
				...
				if ( !empty( $pending_not_validated_investment_campaign_name ) ) {
					NotificationsAPI::kyc_authentified_and_pending_investment( $WDGUserOrOrganization, $pending_not_validated_investment_campaign, $pending_not_validated_investment->get_saved_campaign()->get_api_id() );
